...
public class NEATGATrainingManager {
	private static final Logger logger = LogManager.getLogger(NEATGATrainingManager.class);
	...
	public void initialise(AIConfig config) throws InitialisationFailedException {
		...
		this.random = RandomUtils.getRand();
	 ...
	 */
	public void evolve() {
		int epochs = 0;//Integer.parseInt(config.configElement("NUMBER.EPOCHS"));
		...
		logger.debug("Innovation Database Stats - Hits:" + innovationDatabase.getTotalHits() + " - totalMisses:" + innovationDatabase.getTotalMisses());
	}
...
/*		descriptor.setPAddLink(Double.parseDouble(config.configElement("PROBABILITY.ADDLINK")));
	...
	public ParentSelector createParentSelector(AIConfig config) throws InvalidParentSelectorFunction {
		String pSelectorClass = (String) config.getConfigElementByName("OPERATOR.PSELECTOR");
